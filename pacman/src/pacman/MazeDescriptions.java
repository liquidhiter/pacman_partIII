	...
	public static Maze createMazeFromDescription(Random random, String description) {
		...
		for (int row = 0; row < lines.length; row++) {
			...
			for (int column = 0; column < line.length(); column++) {
				...
				case 'D' -> departurePortals[nbDepaturePortals++] = new DeparturePortal(Square.of(map, row, column));
		...
		return new Maze(random, map, pacMan, 
				Arrays.copyOf(ghosts, nbGhosts), 
				Arrays.copyOf(foodItems, nbFoodItems),
				Arrays.copyOf(departurePortals, nbDepaturePortals), 
				Arrays.copyOf(arrivalPortals, nbArrivalPortals));
	}
